...
import { ChapterRecall } from './Retrieval.jsx'
...
export default function ChapterReader({chId, progress, accent='#38bdf8', onMark, onQuiz, onBack}) {
  ...
  const isLastSection = secIdx === chapter.sections.length - 1
    ...
    <div style={{minHeight:'100vh',background:'#0f172a',color:'#f1f5f9',fontFamily:"'Inter',system-ui,sans-serif",display:'flex',flexDirection:'column'}}>
      <div style={{background:'#1e293b',borderBottom:'1px solid #334155',padding:'12px 20px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
        ...
        <div style={{width:80,height:4,background:'#334155',borderRadius:99,overflow:'hidden'}}>
          <div style={{height:'100%',background:accent,borderRadius:99,width:((secIdx+1)/chapter.sections.length*100)+'%'}}/>
        </div>
      </div>
      ...
      <div style={{display:'flex',flex:1}}>
        <div style={{width:210,background:'#1e293b',borderRight:'1px solid #334155',padding:16,flexShrink:0}}>
          ...
          {allChs.map(ch=>{
            ...
            return (
              <div key={ch.id} style={{display:'flex',alignItems:'center',gap:8,padding:'7px 8px',borderRadius:7,marginBottom:3,background:isActive?'#1e3a5f':'transparent',fontSize:12,color:chRead?'#4ade80':isActive?'#7dd3fc':'#475569',fontWeight:isActive?600:400}}>
                <div style={{width:6,height:6,borderRadius:'50%',background:chRead?'#4ade80':isActive?accent:'#334155',flexShrink:0}}/>
          ...
          {chapter.sections.map((s,i)=>(
            <div key={i} onClick={()=>{if(readSections.includes(i)||i===0||readSections.includes(i-1))setSecIdx(i)}}
              style={{display:'flex',alignItems:'flex-start',gap:6,padding:'5px 6px',borderRadius:6,marginBottom:2,cursor:'pointer',background:secIdx===i?'#0f172a':'transparent',fontSize:11,color:secIdx===i?accent:readSections.includes(i)?'#4ade80':'#475569'}}>
              <div style={{width:5,height:5,borderRadius:'50%',background:readSections.includes(i)?'#4ade80':secIdx===i?accent:'#334155',flexShrink:0,marginTop:3}}/>
        ...
        <div style={{flex:1,overflowY:'auto',padding:'32px 36px',maxWidth:680}}>
          <div style={{fontSize:11,fontWeight:700,color:accent,textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:8}}>Ch {chapter.number} · {section.heading}</div>
          ...
          <div style={{background:'#1e293b',borderRadius:12,border:'1px solid #334155',padding:24,marginBottom:20}}>
            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:16}}>
              ...
              <div style={{fontSize:14,fontWeight:700,color:accent}}>{section.heading}</div>
            </div>
          ...
          {isLastSection && (
            /* Feature B — explain it back. Purely a prompt: it sits above the
               mark-complete control and never gates it. */
            <ChapterRecall key={chId} accent={accent}/>
          )}
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',paddingTop:16,borderTop:'1px solid #1e293b'}}>
            ...
            {secIdx < chapter.sections.length - 1 ? (
              <button onClick={goNext}
                style={{background:accent,color:'#0f172a',border:'none',padding:'9px 20px',borderRadius:8,fontSize:13,fontWeight:700,cursor:'pointer'}}>
                Next →
              </button>
            ) : (
              <button onClick={()=>{if(!isRead)onMark(chId,secIdx)}}
                style={{background:isRead?'#14532d':accent,color:isRead?'#86efac':'#0f172a',border:'none',padding:'9px 20px',borderRadius:8,fontSize:13,fontWeight:700,cursor:'pointer'}}>
          ...
          {allRead && (
            <div style={{background:quizDone?'#14532d':'#1e3a5f',border:`1px solid ${quizDone?'#4ade80':accent}`,borderRadius:10,padding:'16px 18px',marginTop:20,display:'flex',alignItems:'center',justifyContent:'space-between',gap:14}}>
              <div>
              ...
              <button onClick={onQuiz}
                style={{background:quizDone?'#4ade80':accent,color:quizDone?'#14532d':'#0f172a',border:'none',padding:'9px 18px',borderRadius:8,fontSize:12,fontWeight:700,cursor:'pointer',whiteSpace:'nowrap'}}>
